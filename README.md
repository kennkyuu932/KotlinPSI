...
BLEでアドバタイズ，スキャンを行う機能を追加

現状は乱数を名前としてブロードキャストする．

同じ乱数を使用しているはずだが，なぜか表示している乱数とアドバタイズしている名前が一致しない．

仮名を選択しても一回目のBLEの接続がうまくいかないため秘匿共通集合計算までいかない

IPアドレスは送ることができている．

通信相手の選択→確認画面→クライアントにIPアドレスを送る→クライアントがIPアドレスを受取ったことを教える→
秘匿共通集合計算を行う

といった流れにする必要がある?

サーバが通信開始してからクライアントが通信を始める手順は守らなければならない
...
BLE機能を実装するためにアプリ起動時の画面を追加

現状は起動時に表示されるSTARTボタンを押すとPSIのための画面に遷移するのみであり，追加の機能は実装していない．
